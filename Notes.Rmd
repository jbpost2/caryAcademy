...
- Simulate values of $Y$ using an app (in the repo, use `shiny::runGitHub("caryAcademy", username = "jbpost2", subdir = "CardSim", ref = "main")`.
...
- Quick app to look at how the measures work in the app.
...
\color{red}
I'll have code at the ready to update and rerender this plot using the data from minesweeper but I'll show this plot for their version.
\color{black}
```{r echo = FALSE, out.width = "350px", fig.align='center'}
...
\color{red}
If there is time, I'd have them jump into R and run the code to read in the minesweeper data.  Then they could use `lm()` to fit a model and predict.  
\color{black}
